...
const GLIFIC_URL = (
  process.env.GLIFIC_URL || "https://glific.test:3000"
).replace(/\/$/, "");
...
async function runRecipe(recipePath, recordVideo) {
  ...
  for (const flow of recipe.flows) {
    ...
    try {
      ...
      if (flow.required) {
        ...
        console.warn(
          `  Warning: optional flow "${flow.name}" skipped — ${err.message}`
        );
      }
    }
  }
  ...
  await browser.close();
  console.log(
    `  Done → static/img/${recipe.output_dir}/${
      recordVideo ? ` + static/videos/${recipe.output_dir}/` : ""
    }`
  );
}
