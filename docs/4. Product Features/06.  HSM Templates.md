...
This opens the **HSM Templates** list, which shows every template created in your organization.

![HSM Templates list](../../static/img/hsm-templates/hsm-templates_list.png)

Each row shows:

- `Element name` - the WABA namespace title you gave the template. Select the arrow next to the name to expand it and see every language variant that shares the same element name, along with a preview of each variant's message.

  ![Expanded language variants](../../static/img/hsm-templates/hsm-templates_list_language_variants.png)

- `Languages` - a chip for each language the template has been submitted in. The colored dot shows that variant's status - green for `Approved`, yellow for `Pending`, and red for `Rejected` or `Failed`. Hover over a chip to see its exact status.
- `Category` - the WhatsApp category (e.g. `Utility`, `Marketing`) the template was submitted under. If a template has variants in more than one category, all of them are listed.
- `Last updated` - how long ago the template was last modified. This column changes to `Reason` when you filter by `Rejected` or `Failed`, showing why WhatsApp rejected that template.
- `Actions` - quick actions for that template: `Add new language` (submit a translated variant under the same element name), `View` (open the template), `Copy` (duplicate it as a new template), and `Copy UUID` (copy the template's ID to your clipboard).

**Filtering the list**

Use the filters above the table to narrow down the list:

- The status dropdown filters by `All`, `Approved`, `Pending`, `Rejected`, or `Failed`.

  ![Filtering by status](../../static/img/hsm-templates/hsm-templates_list_status_filter.png)

- The category dropdown filters by WhatsApp category.
- The tag dropdown filters templates by the tag assigned to them.

**Keeping template status up to date**

Select `SYNC HSM` at the top of the page to refresh the approval status of all `Pending` templates from WhatsApp. Since approvals can take anywhere from a couple of minutes to 48 hours, use this whenever you're checking on a recently submitted template.

---

## HSM Template Creation & Approval Submission

1. From the Templates list page, click on `+ CREATE` button from the top right corner.
