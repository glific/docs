...
Accepts the input if it contains all of the words listed in the criteria, in any order. Multiple words can be added, separated by commas.
Example: Criteria = `red, apple` → "I want a red apple" ✅, "I want a red banana" ❌ (missing "apple").

### 3. has a phrase
Accepts the input if the exact phrase (the words in that order) appears anywhere in the response.
Example: Criteria = `good morning` → "good morning, how are you?" ✅, "morning, good sir" ❌ (words out of order).

### 4. has only the phrase
Accepts the input only if it matches the criteria exactly, with nothing else added.
Example: Criteria = `good morning` → "good morning" ✅, "good morning!" or "good morning to you" ❌.
