...
This option accepts the input only if it contains all of the words defined in the response criteria, in any order. Multiple words can be added, separated by commas.
For example, if the criteria is red, apple, then "I want a red apple" is accepted, but "I want a red banana" is not, since it is missing the word "apple".

### 3. has a phrase
This option accepts the input if the exact phrase (the words in that order) appears anywhere in the response.
For example, if the criteria is good morning, then "good morning, how are you?" is accepted, but "morning, good sir" is not, since the words are out of order.

### 4. has only the phrase
This option accepts the input only if it matches the criteria exactly, with nothing else added.
For example, if the criteria is good morning, then "good morning" is accepted, but "good morning!" or "good morning to you" is not.
