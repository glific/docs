...
For example, if the criteria is order, then "order 123" will be treated as valid, but "my order is 123" will not, since it does not start with "order".
...
To repeat a reminder like this, connect the no-response path back into another Wait for Response node with its own timer, chaining as many times as you like. See "The no-response branch in your flow" below for what this branch looks like on the canvas.
