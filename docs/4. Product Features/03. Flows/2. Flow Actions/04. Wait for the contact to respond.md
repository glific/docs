...
- You can add more than one row of criteria. Each row becomes its own branch, so a reply can be routed differently depending on which criteria it matches.
...
- **Continue when there is no response for**: optional. Tick this box and pick a time gap (for example, 1 hour) to let the flow move on to a separate path if the contact never replies, instead of waiting forever. See Handling no response below.
...
![Response type dropdown showing has any of the words, has all of the words, has the phrase, and has only the phrase](../../../../static/img/flows/flows_wait_for_response_criteria.png)
...
For example, if the criteria is customer support, then "I need customer support please" will be treated as valid, but "I need support from a customer" will not, since the words are not in that order.
...
For example, if the criteria is yes please, then "yes please" will be treated as valid, but "yes please now" will not, since it has an extra word.
...
- for the media file URL: `@results.picture.url`
- for the media file caption: `@results.picture.caption`
...
In this example, the syntax is:
- for the audio file URL: `@results.audio.url`
- for the audio file caption: `@results.audio.caption`
...
In this example, the syntax is:
- for the video file URL: `@results.video.url`
- for the video file caption: `@results.video.caption`
...
In this example, the syntax is:
- for the image file URL: `@results.image.url`
- for the image file caption: `@results.image.caption`
...
In this example, the syntax is:
- for the file URL: `@results.file.url`
- for the file caption: `@results.file.caption`
...
Tick **Continue when there is no response for** and choose the time gap after which the flow should move on. In the example below, the flow continues down the no-response path if the contact has not replied within 1 hour.

To send a repeated reminder (for example, every hour, up to 3 times) rather than moving on after a single wait, connect this no-response path back into another Wait for Response node with its own timer. See "The no-response branch in your flow" below for what this looks like on the canvas.
