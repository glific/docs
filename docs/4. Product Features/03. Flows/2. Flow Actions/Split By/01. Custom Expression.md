...
4. Syntax to check name contains only english characters
`<%= Regex.match?(~r/^[A-Za-z]+$/, name) %>`

This returns a boolean value:

- true → if the input contains only English letters (A–Z, a–z).
- false → if the input contains numbers, spaces, or any special characters.

Replace name with the variable that stores the captured user input.
